...
#define CMD2_VAR_LIST(key)                                              \
  ...
  CMD2_ANY_VOID(key ".push_back", ([storage = control->object_storage(), k = torrent::raw_string::from_c_str(key)](const auto&, const auto& v) { return storage->list_push_back(k, v); }));
